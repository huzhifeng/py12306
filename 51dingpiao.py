...
def stationInit():
  ...
  try:
    ...
    print(u"stationInit()->sendGetRequest(%s)->resp.read() exception"%(url))
...
def getQueueCount(hiddenDict):
  ...
  try:
    ...
    print(u"getQueueCount(%s)->eval(respInfo) exception"%(url))
    return 0
  ...
  print(u"目前排队人数：%d，在你之前的排队人数：%d"%(respDict['countT'], respDict['count']))
  if respDict['op_2']:
